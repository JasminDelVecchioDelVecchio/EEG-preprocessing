function [EEG] = performDetrend(EEG,params)
    fprintf('Performing detrending...\n');
    [EEG.data] = (mydetrend(EEG.data',round(params.winsize*EEG.srate),round((params.winoverlap)/100*params.winsize*EEG.srate))');
    EEG.pipeline.detrending.performed = 'yes';
    EEG.pipeline.detrending.winsize = params.winsize;
    EEG.pipeline.detrending.winstep = params.winoverlap;
end

function [y]=mydetrend(x,wsize,overlap,order,thresh,niter,w)
...
elseif wsize==dims(1)
    ...
    for iIter=1:niter
        ...
        striter = sprintf('\tIteration %d/%d : ',iIter,niter);
        textbar = textprogressbar(striter);
...
%         overlap = wsize/2;
        ...
        while true
            ...
            textbar.updatebar(round(100*offset/(size(x,1)-winstep)));
            start=offset+1;
            stop=min(size(x,1),offset+wsize);

%             if not enough valid samples grow window:
            counter=5;
            while any (sum(w(start:stop,:)) < ceil(wsize/20))
                if counter <= 0 ; break; end 
                start=max(1,start-ceil(wsize/10));
                stop=min(size(x,1),stop+ceil(wsize/10));
                counter=counter-1;
            end
...
%                 wt=tukeywin(wsize2,2*overlap/wsize2);
...
%                 b=tukeywin(wsize2,2*overlap/wsize2);
            end
