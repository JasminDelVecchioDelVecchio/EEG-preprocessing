function [x, FiltSpec, Messages] = art_bandpass_hfilter(x,lp,hp,fs, isMirror, isRelax, Function, Rolloff)
             %
            % DESCRIPTION:
            ...
            if isempty(FiltSpec)
                ...
                % High-pass filter
                if ~isempty(hp) && (hp ~= 0)
                    f_highpass = hp / Nyquist;    % Change frequency from Hz to normalized scale (0-1)
                    % Default transition band
                    if isempty(Rolloff)
                        if (hp <= 5)   % Relax the transition band if HighPass<5 Hz
                        ...
                        f_highstop = max(0.2, hp - Rolloff) / Nyquist;
                    end
                else
                    f_highpass = 0;
                    f_highstop = 0;
                ...
                if ~isempty(lp) && (lp ~= 0)
                    ...
                    if isempty(Rolloff)
                        ...
                        f_lowstop = min(Nyquist - 0.2, lp + Rolloff) / Nyquist;
                    end
                else
                    f_lowpass  = 0;
                    f_lowstop  = 0;
