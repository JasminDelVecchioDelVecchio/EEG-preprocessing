...
[~, EEGCleaned,referenceOut] = evalc('Reference(EEGCleaned, params,remove_range)');
...
interpolatedChannel = false(1,EEGCleaned.nbchan);
if(isfield(referenceOut, 'interpolatedChannels'))
    interpchan=referenceOut.interpolatedChannels;
        if(isfield(interpchan, 'all'))
            interpolatedChannelNumbers = interpchan.all;
            interpolatedChannel(interpolatedChannelNumbers) = true;
        end
end
EEG.badchan = removedMask;
% EEG.interpolatedchan(~oldremovedMask) = interpolatedChannel;
EEGCleaned.data(interpolatedChannel,:) = referenceOut.badSignalsUninterpolated;
