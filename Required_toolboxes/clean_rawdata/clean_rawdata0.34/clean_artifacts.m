...
if ~strcmp(burst_crit,'off')
    oldEEG = EEG;
    EEG = clean_asr(oldEEG,burst_crit,[],[],[],burst_crit_refmaxbadchns,burst_crit_reftolerances,[]);
    num_iter = 1;
    while sqrt(mean(mean(((EEG.data - oldEEG.data)./oldEEG.data).^2)))>0 && num_iter < 5
        oldEEG = EEG;
        EEG = clean_asr(oldEEG,burst_crit,[],[],[],burst_crit_refmaxbadchns,burst_crit_reftolerances,[]); 
        num_iter = num_iter +1;
    end
...
if ~strcmp(window_crit,'off') && ~strcmp(window_crit_tolerances,'off')
    ...
    [~,sample_mask] = clean_windows(EEG,window_crit,window_crit_tolerances,window_length,window_overlap);
    EEG.etc.clean_sample_mask = sample_mask;
end

if isfield(EEG.etc,'clean_channel_mask')
    tmp = zeros(orig_size);
    badchan = setdiff(1:orig_size(1),find(EEG.etc.clean_channel_mask));
    tmp(EEG.etc.clean_channel_mask,:) = EEG.data;
    tmp(badchan,:) = HP.data(badchan,:);
    EEG.data = tmp;
    EEG.nbchan = orig_size(1);
end
