...
sub_size = round(subset_size*size(signal.data,1));
...
usable_channels = (~cellfun('isempty',x) & ~cellfun('isempty',y) & ~cellfun('isempty',z))';

removed_channels = (noise_mask');

locs = [cell2mat(x);cell2mat(y);cell2mat(z)];

rem_chan = 1;
while(any(rem_chan))
    good_channels = (~removed_channels);
    selected_channels = usable_channels&good_channels;
    sze = sum(selected_channels);
    % get the matrix of all channel locations [3xN]
    sub_size = round(subset_size*sze);
    % caculate all-channel reconstruction matrices from random channel subsets
    P = hlp_microcache('cleanchans',@calc_projector,locs(:,selected_channels),num_samples,sub_size);
    corrs = zeros(sze,W);
    ...
    for o=1:W
        tic; % makoto
        XX = X(offsets(o)+wnd,selected_channels);
        YY = sort(reshape(XX*P,window_len,sze,num_samples),3);
        YY = YY(:,:,round(end/2));
    ...
    % ignored samples
    rem_chan = false(C,1);
    rem_chan(selected_channels) = sum(flagged,2)*window_len > max_broken_time;
    removed_channels = removed_channels | rem_chan;
end
% apply removal
